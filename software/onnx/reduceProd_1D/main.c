...
int32_t Partial_prods[NUM_CORES] __attribute__((section(".l1")))
__attribute__((aligned(NUM_CORES * 4 * 4)));
...
int32_t reduce_prod_parallel1(int32_t const *__restrict__ A,
...
int32_t reduce_prod_parallel2(int32_t const *__restrict__ A,
                              uint32_t num_elements, uint32_t id,
                              uint32_t numThreads) {
...
int32_t reduce_prod_parallel3(int32_t const *__restrict__ A,
                              uint32_t num_elements, uint32_t id,
                              uint32_t numThreads) {
  ...
  if (id == 0) {
    ...
    for (uint32_t i = id; i < 64; i += 1) {
      reduced4[0] *= Partial_prods[i];
    }
  }
  if (id == 16) {
    reduced4[1] = 1;
    for (uint32_t i = 64; i < 128; i += 1) {
      reduced4[1] *= Partial_prods[i];
    }
  }
  if (id == 32) {
    reduced4[2] = 1;
    for (uint32_t i = 128; i < 192; i += 1) {
      reduced4[2] *= Partial_prods[i];
    }
  }
  if (id == 48) {
    reduced4[3] = 1;
    for (uint32_t i = 192; i < 256; i += 1) {
      reduced4[3] *= Partial_prods[i];
    }
  }
  mempool_barrier(numThreads);
  if (id == 0) {
...
int32_t reduce_prod_parallel4(int32_t const *__restrict__ A,
                              uint32_t num_elements, uint32_t id,
                              uint32_t numThreads) {
  ...
  if (id == 0) {
    ...
    for (uint32_t j = 0; j < 16; j += 1) {
      reduced16[0] *= Partial_prods[j];
    }
  }
  if (id == 4) {
    reduced16[1] = 1;
    for (uint32_t j = 16; j < 32; j += 1) {
      reduced16[1] *= Partial_prods[j];
    }
  }
  if (id == 8) {
    reduced16[2] = 1;
    for (uint32_t j = 32; j < 48; j += 1) {
      reduced16[2] *= Partial_prods[j];
    }
  }
  if (id == 12) {
    reduced16[3] = 1;
    for (uint32_t j = 48; j < 64; j += 1) {
      reduced16[3] *= Partial_prods[j];
    }
  }
  if (id == 16) {
    reduced16[4] = 1;
    for (uint32_t j = 64; j < 80; j += 1) {
      reduced16[4] *= Partial_prods[j];
    }
  }
  if (id == 20) {
    reduced16[5] = 1;
    for (uint32_t j = 80; j < 96; j += 1) {
      reduced16[5] *= Partial_prods[j];
    }
  }
  if (id == 24) {
    reduced16[6] = 1;
    for (uint32_t j = 96; j < 112; j += 1) {
      reduced16[6] *= Partial_prods[j];
    }
  }
  if (id == 28) {
    reduced16[7] = 1;
    for (uint32_t j = 112; j < 128; j += 1) {
      reduced16[7] *= Partial_prods[j];
    }
  }
  if (id == 32) {
    reduced16[8] = 1;
    for (uint32_t j = 128; j < 144; j += 1) {
      reduced16[8] *= Partial_prods[j];
    }
  }
  if (id == 36) {
    reduced16[9] = 1;
    for (uint32_t j = 144; j < 160; j += 1) {
      reduced16[9] *= Partial_prods[j];
    }
  }
  if (id == 40) {
    reduced16[10] = 1;
    for (uint32_t j = 160; j < 176; j += 1) {
      reduced16[10] *= Partial_prods[j];
    }
  }
  if (id == 44) {
    reduced16[11] = 1;
    for (uint32_t j = 176; j < 192; j += 1) {
      reduced16[11] *= Partial_prods[j];
    }
  }
  if (id == 48) {
    reduced16[12] = 1;
    for (uint32_t j = 192; j < 208; j += 1) {
      reduced16[12] *= Partial_prods[j];
    }
  }
  if (id == 52) {
    reduced16[13] = 1;
    for (uint32_t j = 208; j < 224; j += 1) {
      reduced16[13] *= Partial_prods[j];
    }
  }
  if (id == 56) {
    reduced16[14] = 1;
    for (uint32_t j = 224; j < 240; j += 1) {
      reduced16[14] *= Partial_prods[j];
    }
  }
  if (id == 60) {
    reduced16[15] = 1;
    for (uint32_t j = 240; j < 256; j += 1) {
      reduced16[15] *= Partial_prods[j];
    }
  }
  mempool_barrier(numThreads);
  if (id == 0) {
...
int32_t reduce_prod_parallel_atomic(int32_t const *__restrict__ A,
                                    uint32_t num_elements, uint32_t id,
                                    uint32_t numThreads) {
...
int main() {
  ...
  cycles = mempool_get_timer();
  mempool_start_benchmark();
  result = reduce_prod_parallel2(a, M, core_id, num_cores);
  mempool_stop_benchmark();
  cycles = mempool_get_timer() - cycles;
  ...
  if (core_id == 0) {
    ...
    cycles = mempool_get_timer();
    mempool_start_benchmark();
    omp_result = reduce_prod_omp_static(a, M);
    mempool_stop_benchmark();
    cycles = mempool_get_timer() - cycles;
    ...
    // cycles = mempool_get_timer();
    // mempool_start_benchmark();
    // omp_result = reduce_prod_omp_dynamic(a, IS);
    // mempool_stop_benchmark();
    // cycles = mempool_get_timer() - cycles;
    ...
    // cycles = mempool_get_timer();
    // mempool_start_benchmark();
    // omp_result = reduce_prod_omp_dynamic(a, 16);
    // mempool_stop_benchmark();
    // cycles = mempool_get_timer() - cycles;
