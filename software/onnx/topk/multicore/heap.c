...
void heapify_max(heap_t *heap, uint32_t i){
    /*
...
void heapify_min(heap_t *heap, uint32_t i){
    /*
     * Heapify function: takes a heap and the index of a node of it, 
     * and moves it to the correct position in the heap.
     * 
    ...
    */
    ...
    if(rchild_idx < heap_size){
    	uint32_t rchild = heap->data[rchild_idx];
    	if(heap->data[largest] < rchild){
    		largest = rchild_idx;
    	}
    }
    if(largest != i){
    	swap(&heap->data[i], &heap->data[largest]);
    	heapify_min(heap, largest);
    }
}

void heapsort(heap_t *heap) {
    uint32_t heap_size = heap->size;
    for(int32_t i = ((int32_t) heap_size) - 1; i > 0; i--) {
        swap(&(heap->data[0]), &(heap->data[i]));
        heap->size = (uint32_t) i;
        heapify_max(heap, 0);
    }
