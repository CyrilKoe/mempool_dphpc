...
int32_t reduce_Min_wrapper(int32_t const *__restrict__ data,
  ...
  if (num_axes == 0) {
    if (noop_with_empty_axes) {
      ...
      error = reduce_Min_all(data, shape, rank, reduced);
    }
  } else {
    if (rank == 1) {
      error = reduce_Min_all(data, shape, rank, reduced);
      ...
      if (num_axes == 1){
        // printf("Start 2d_1ax...\n");
        error = reduce_Min_4d_1ax(data, inter_shape, axes[0], reduced);
      } else if (num_axes == 2){
        // printf("Start 2d_2ax...\n");
        error = reduce_Min_all(data, shape, rank, reduced);
      } else {
      ...
      if (num_axes == 1){
        ...
        // printf("Start 3d_2ax...\n");
        error = reduce_Min_4d_2ax(data, inter_shape, axes, reduced);
      } else if (num_axes == 3){
        // printf("Start 3d_3ax...\n");
        error = reduce_Min_all(data, shape, rank, reduced);
      } else {
      ...
      if (num_axes == 1){
        // printf("Start 4d_1ax...\n");
        error = reduce_Min_4d_1ax(data, shape, axes[0], reduced);
      } else if (num_axes == 2){
        // printf("Start 4d_2ax...\n");
        error = reduce_Min_4d_2ax(data, shape, axes, reduced);
      } else if (num_axes == 3){
        // printf("Start 4d_3ax...\n");
        error = reduce_Min_4d_3ax(data, shape, axes, reduced);
      } else if (num_axes == 4){
        // printf("Start 4d_4ax...\n");
        error = reduce_Min_all(data, shape, rank, reduced);
      } else {
...
// #endif
  ...
  if (core_id == 0) {
    ...
    error = reduce_Min_wrapper(data, shape, axes, rank, num_axes, 1, noop_with_empty_axes);
