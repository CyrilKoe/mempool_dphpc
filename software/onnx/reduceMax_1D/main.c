...
int32_t Partial_maxs[NUM_CORES] __attribute__((section(".l1")))
__attribute__((aligned(NUM_CORES * 4 * 4)));
...
int32_t reduce_max_sequential(int32_t const *__restrict__ A,
...
int32_t reduce_max_parallel1(int32_t const *__restrict__ A,
  ...
  if (id == 0) {
    for (uint32_t i = 0; i < numThreads; i += 1) {
      if (Partial_maxs[i] > reduced)
        reduced = Partial_maxs[i];
    }
  }
  mempool_barrier(numThreads);
  return reduced;
...
int32_t reduce_max_parallel3(int32_t const *__restrict__ A,
  ...
  if (id == 0) {
    ...
    for (uint32_t i = id; i < 64; i += 1) {
      if (Partial_maxs[i] > reduced4[0])
        reduced4[0] = Partial_maxs[i];
    }
  }
  if (id == 16) {
    reduced4[1] = MIN32;
    for (uint32_t i = 64; i < 128; i += 1) {
      if (Partial_maxs[i] > reduced4[1])
        reduced4[1] = Partial_maxs[i];
    }
  }
  if (id == 32) {
    reduced4[2] = MIN32;
    for (uint32_t i = 128; i < 192; i += 1) {
      if (Partial_maxs[i] > reduced4[2])
        reduced4[2] = Partial_maxs[i];
    }
  }
  if (id == 48) {
    reduced4[3] = MIN32;
    for (uint32_t i = 192; i < 256; i += 1) {
      if (Partial_maxs[i] > reduced4[3])
        reduced4[3] = Partial_maxs[i];
    }
  }
  mempool_barrier(numThreads);
  if (id == 0) {
...
int32_t reduce_max_parallel4(int32_t const *__restrict__ A,
  ...
  if (id == 0) {
    ...
    for (uint32_t j = 0; j < 16; j += 1) {
      if (Partial_maxs[j] > reduced16[0])
        reduced16[0] = Partial_maxs[j];
    }
  }
  if (id == 4) {
    reduced16[1] = MIN32;
    for (uint32_t j = 16; j < 32; j += 1) {
      if (Partial_maxs[j] > reduced16[1])
        reduced16[1] = Partial_maxs[j];
    }
  }
  if (id == 8) {
    reduced16[2] = MIN32;
    for (uint32_t j = 32; j < 48; j += 1) {
      if (Partial_maxs[j] > reduced16[2])
        reduced16[2] = Partial_maxs[j];
    }
  }
  if (id == 12) {
    reduced16[3] = MIN32;
    for (uint32_t j = 48; j < 64; j += 1) {
      if (Partial_maxs[j] > reduced16[3])
        reduced16[3] = Partial_maxs[j];
    }
  }
  if (id == 16) {
    reduced16[4] = MIN32;
    for (uint32_t j = 64; j < 80; j += 1) {
      if (Partial_maxs[j] > reduced16[4])
        reduced16[4] = Partial_maxs[j];
    }
  }
  if (id == 20) {
    reduced16[5] = MIN32;
    for (uint32_t j = 80; j < 96; j += 1) {
      if (Partial_maxs[j] > reduced16[5])
        reduced16[5] = Partial_maxs[j];
    }
  }
  if (id == 24) {
    reduced16[6] = MIN32;
    for (uint32_t j = 96; j < 112; j += 1) {
      if (Partial_maxs[j] > reduced16[6])
        reduced16[6] = Partial_maxs[j];
    }
  }
  if (id == 28) {
    reduced16[7] = MIN32;
    for (uint32_t j = 112; j < 128; j += 1) {
      if (Partial_maxs[j] > reduced16[7])
        reduced16[7] = Partial_maxs[j];
    }
  }
  if (id == 32) {
    reduced16[8] = MIN32;
    for (uint32_t j = 128; j < 144; j += 1) {
      if (Partial_maxs[j] > reduced16[8])
        reduced16[8] = Partial_maxs[j];
    }
  }
  if (id == 36) {
    reduced16[9] = MIN32;
    for (uint32_t j = 144; j < 160; j += 1) {
      if (Partial_maxs[j] > reduced16[9])
        reduced16[9] = Partial_maxs[j];
    }
  }
  if (id == 40) {
    reduced16[10] = MIN32;
    for (uint32_t j = 160; j < 176; j += 1) {
      if (Partial_maxs[j] > reduced16[10])
        reduced16[10] = Partial_maxs[j];
    }
  }
  if (id == 44) {
    reduced16[11] = MIN32;
    for (uint32_t j = 176; j < 192; j += 1) {
      if (Partial_maxs[j] > reduced16[11])
        reduced16[11] = Partial_maxs[j];
    }
  }
  if (id == 48) {
    reduced16[12] = MIN32;
    for (uint32_t j = 192; j < 208; j += 1) {
      if (Partial_maxs[j] > reduced16[12])
        reduced16[12] = Partial_maxs[j];
    }
  }
  if (id == 52) {
    reduced16[13] = MIN32;
    for (uint32_t j = 208; j < 224; j += 1) {
      if (Partial_maxs[j] > reduced16[13])
        reduced16[13] = Partial_maxs[j];
    }
  }
  if (id == 56) {
    reduced16[14] = MIN32;
    for (uint32_t j = 224; j < 240; j += 1) {
      if (Partial_maxs[j] > reduced16[14])
        reduced16[14] = Partial_maxs[j];
    }
  }
  if (id == 60) {
    reduced16[15] = MIN32;
    for (uint32_t j = 240; j < 256; j += 1) {
      if (Partial_maxs[j] > reduced16[15])
        reduced16[15] = Partial_maxs[j];
    }
  }
  mempool_barrier(numThreads);
  if (id == 0) {
...
int32_t reduce_max_omp_static(int32_t const *__restrict__ A,
                              uint32_t num_elements) {
  uint32_t i;
  int32_t reduced = MIN32;
#pragma omp parallel for reduction(max : reduced)
  for (i = 0; i < num_elements; i++) {
    if (A[i] > reduced)
      reduced = A[i];
  }
...
int32_t reduce_max_omp_dynamic(int32_t const *__restrict__ A,
                               uint32_t chunksize) {
  uint32_t i;
  int32_t reduced = MIN32;
  // printf("num_elements %d\n", num_elements);
#pragma omp parallel for schedule(dynamic, chunksize) reduction(max : reduced)
  for (i = 0; i < M; i++) {
    if (A[i] > reduced)
      reduced = A[i];
  }
...
int main() {
  ...
  cycles = mempool_get_timer();
  mempool_start_benchmark();
  result = reduce_max_parallel2(a, M, core_id, num_cores);
  mempool_stop_benchmark();
  cycles = mempool_get_timer() - cycles;
  ...
  if (core_id == 0) {
    ...
    // cycles = mempool_get_timer();
    // mempool_start_benchmark();
    // result = reduce_max_omp_parallel_critical(a, M/4, num_cores);
    // mempool_stop_benchmark();
    // cycles = mempool_get_timer() - cycles;
    ...
    cycles = mempool_get_timer();
    mempool_start_benchmark();
    omp_result = reduce_max_omp_static(a, M);
    mempool_stop_benchmark();
    cycles = mempool_get_timer() - cycles;
    ...
    // cycles = mempool_get_timer();
    // mempool_start_benchmark();
    // omp_result = reduce_max_omp_dynamic(a, IS);
    // mempool_stop_benchmark();
    // cycles = mempool_get_timer() - cycles;
    ...
    // cycles = mempool_get_timer();
    // mempool_start_benchmark();
    // omp_result = reduce_max_omp_dynamic(a, 16);
    // mempool_stop_benchmark();
    // cycles = mempool_get_timer() - cycles;
