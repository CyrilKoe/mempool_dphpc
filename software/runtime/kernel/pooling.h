...
// they should be actually given via function call
#define K_END 16
#define STRIDE 4
#define M_DIM 268 // (OUT - 1) * S + K
#define X_START 0
...
// dump(checksum, 8);
// dump(index, 9);
// Each core computes the output matrix. No parallelization yet for benchmark
// reference.
static inline void max_pooling_sequential(int32_t const *__restrict__ A,
    ...
    for (uint32_t y = 0; y < M - K + 1; y += S) {
      ...
      for (uint32_t k_x = 0; k_x < K; k_x++) {
        for (uint32_t k_y = 0; k_y < K / 4; k_y += 4) {
          ...
          max0 = A[y + k_y + 0 + (x + k_x) * M] > max0
                     ? A[y + k_y + 0 + (x + k_x) * M]
                     : max0;
          max1 = A[y + k_y + 1 + (x + k_x) * M] > max1
                     ? A[y + k_y + 1 + (x + k_x) * M]
                     : max1;
          max2 = A[y + k_y + 2 + (x + k_x) * M] > max2
                     ? A[y + k_y + 2 + (x + k_x) * M]
                     : max2;
          max3 = A[y + k_y + 3 + (x + k_x) * M] > max3
                     ? A[y + k_y + 3 + (x + k_x) * M]
                     : max3;
        }
      }
...
void max_pooling_parallel(int32_t const *__restrict__ A,
                          int32_t *const __restrict__ B, uint32_t Mx,
                          uint32_t My, uint32_t K, uint32_t S, uint32_t core_id,
                          uint32_t num_cores) {
    ...
    for (uint32_t x = x_start; x < x_end; x += S * num_cores / x_split) {
      ...
      for (uint32_t k_y = 0; k_y < K; k_y++) {
        for (uint32_t k_x = 0; k_x < K; k_x += 4) {
          // if (A[x + k_x + (y + k_y) * Mx] > max) { // change to x + k_x + (y
          // + k_y) * Mx
          //     max = A[x + k_x + (y + k_y) * Mx]; // change to x + k_x + (y +
          //     k_y) * Mx
          // }

          max_0 = A[x + k_x + 0 + (y + k_y) * Mx] > max_0
                      ? A[x + k_x + 0 + (y + k_y) * Mx]
                      : max_0;
          max_1 = A[x + k_x + 1 + (y + k_y) * Mx] > max_1
                      ? A[x + k_x + 1 + (y + k_y) * Mx]
                      : max_1;
          max_2 = A[x + k_x + 2 + (y + k_y) * Mx] > max_2
                      ? A[x + k_x + 2 + (y + k_y) * Mx]
                      : max_2;
          max_3 = A[x + k_x + 3 + (y + k_y) * Mx] > max_3
                      ? A[x + k_x + 3 + (y + k_y) * Mx]
                      : max_3;
        }
      }
      ...
      B[(int)(x / S) + (int)(y / S) * ((int)((Mx - K) / S) + 1)] =
          max_0; // change to x/S + y/S * (Mx - K + 1)/S
                 // dump_max(max);
    }
  }
}

static inline void max_pooling_openmp_static(int32_t const *__restrict__ A,
                                             int32_t *const __restrict__ B,
...
#pragma omp parallel for num_threads(THREADS) collapse(2)                      \
    firstprivate(max) // num_threads(NUM_THREADS)
...
static inline void max_pooling_openmp_dynamic(int32_t const *__restrict__ A,
                                              int32_t *const __restrict__ B,
                                              uint32_t M, uint32_t K,
                                              uint32_t S) {
