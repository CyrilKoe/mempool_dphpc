    ...
    for files in os.listdir(path):
        if files.endswith(ext):
            ...
            for section in set(csvread['section']):
                ...
                cycles[section] = max
            df_tmp = pd.DataFrame(cycles, index=['seq', 'man1', 'man2', 'man3', 'man4', 'seq_simd', 'man1_simd', 'man2_simd', 'man3_simd', 'man4_simd', 'omp_stat'], columns=[folders])
            df = pd.concat([df, df_tmp], axis=1)
    os.chdir(main_path)
print(df)
df.to_csv('../ReduceSum8.csv')